...
# 在visualize.py文件中，img_poly=coarse_polys
...
# 在visualize.py文件中，polys=coarse_polys * self.down_sample，ro=4
def prepare_testing_init(polys, ro):
    polys = polys / ro  # 相当于抵消down_sample的作用
    can_init_polys = img_poly_to_can_poly(polys)  # can_init_polys的形状和polys相同
    img_init_polys = polys
    ind = torch.zeros((img_init_polys.size(0), ), dtype=torch.int32, device=img_init_polys.device)  # ind =[4., 4., 4., 4.]
